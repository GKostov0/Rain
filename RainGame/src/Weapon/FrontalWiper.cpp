...
namespace rn
{
	FrontalWiper::FrontalWiper(Actor* owner, float cooldownTime,
		...
		_shooter5{ new BulletShooter{ owner, cooldownTime / 1.5f, {localOffset.x, localOffset.y + width / 1.5f}, 2.0f, "SpaceShooterRedux/PNG/Lasers/laserBlue01.png"} },
		_shooter6{ new BulletShooter{ owner, cooldownTime / 1.5f, {localOffset.x, localOffset.y - width / 1.5f}, -2.0f, "SpaceShooterRedux/PNG/Lasers/laserBlue01.png"} }
	{
	...
	void FrontalWiper::SetCurrentLevel(int level)
	{
		...
		_shooter5->SetCurrentLevel(level);
		_shooter6->SetCurrentLevel(level);
	}
